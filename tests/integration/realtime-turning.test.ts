...
test("barge-in cancels active agent response and clears speaking state", async () => {
  ...
  assert.ok((await eventSink.list()).find((event) => event.type === "agent.response.cancelled"));
});

test("duplicate provider events are ignored to prevent duplicate downstream actions", async () => {
  const eventSink = new InMemoryEventSink();
  const simulator = new RealtimeTurnSimulator("sess_turn", "tenant_turn", eventSink);
  ...
  assert.equal((await eventSink.list()).filter((event) => event.type === "turn.detected").length, 1);
});
